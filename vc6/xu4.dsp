...
PostBuild_Cmds=@ECHO OFF	set U4PATH=c:\xu4	IF EXIST %U4PATH% (	echo Install directory set to %U4PATH%	IF NOT EXIST %U4PATH% mkdir %U4PATH%	IF NOT EXIST %U4PATH%\conf mkdir %U4PATH%\conf	IF NOT EXIST %U4PATH%\conf\dtd mkdir %U4PATH%\conf\dtd	IF NOT EXIST %U4PATH%\graphics mkdir %U4PATH%\graphics	IF NOT EXIST %U4PATH%\mid mkdir %U4PATH%\mid	IF NOT EXIST %U4PATH%\sound mkdir %U4PATH%\sound	@ECHO ON	xcopy ..\lib\*.dll %U4PATH%\. /E /D /Y	xcopy ..\conf\*.xml %U4PATH%\conf\. /E /D /Y	xcopy ..\conf\dtd\*.dtd %U4PATH%\conf\dtd\. /E /D /Y	xcopy ..\graphics %U4PATH%\graphics\. /E /D /Y	xcopy ..\mid %U4PATH%\mid\. /E /D /Y	xcopy ..\sound %U4PATH%\sound\. /E /D /Y	) ELSE (	echo An error occurred while setting the xu4 installation path.	)
